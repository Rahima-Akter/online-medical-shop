...
export default async function FeaturedMedicines() {
  const { medicines } = await getAllMedicine(1, 1000);
  ...
  const products: Medicine[] = allMed
    .map((med: Medicine) => ({
      ...med,
      avgRating:
        med.reviews.reduce((a, r) => a + r.rating, 0) / med.reviews.length || 0,
    }))
    .filter((med: any) => med.avgRating >= 4)
    .slice(0, 4);
      ...
      <div className="max-w-360 mx-auto px-4 md:px-10 lg:px-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {products && products.length > 0 ? (
            ...
            <div className="col-span-4 flex flex-row items-center justify-center font-label uppercase tracking-widest text-[#EBBA92]">
              <PriorityHigh className="text-2xl text-red-500" />
              <p className="text-sm font-bold">No Featured Medicines Found</p>
            </div>
          )}
        </div>
